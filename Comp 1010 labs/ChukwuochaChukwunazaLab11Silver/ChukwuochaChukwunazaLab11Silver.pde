 ...
 * Silver exercise
 * Fall Lab 11 - COMP 1010[A02]
 * 
 * Simplified Yahtzee game - phase 5
 * Test the maxInARow and hasFullHouse functions
 * To make a better test, 10 dice are used instead of only 5,
 * and they're "9 sided" (well, capable of displaying 1-9 dots anyway).
 * 
 */
...
final int NUM_SIDES=9;   //Sides on the dice
final int NUM_DICE=10;    //The number of dice used
...
//Insert your Bronze code here

int[] freqCount(int[] roll) { //2,6,2,1,4
...
int maxInARow(int[] freqs) {
  ...
  for (int i=0; i<freqs.length; i++)
  {
    if (freqs[i] != 0)
    {
      counter++;
      println(counter);
      maxNonZero = max(maxNonZero, counter);
    } else
    {
      counter = 0;
    }
...
boolean hasFullHouse(int[] freqs) {
  ...
  boolean freq3Exists = false;
  boolean freq2Exists = false;
...
void mouseClicked() {
  ...
  rollDice();
  showDiceRoll();
  displayStats(numDice); //*****CHANGE TO THE NAME OF YOUR VARIABLE
}
...
void displayStats(int[] theDice) {
  //Test the freqCount, maxOfAKind, maxInARow and hasFullHouse
  //functions by calling them and displaying the results as a message.
  ...
  message += "There are "+maxInARow(freqs)+" non-zero values in a row.\n";
  message += "There is "+(hasFullHouse(freqs)?"":"not ")+"a full house.\n";
