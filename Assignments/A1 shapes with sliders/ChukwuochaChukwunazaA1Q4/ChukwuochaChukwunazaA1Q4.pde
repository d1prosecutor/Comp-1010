...
//Constants for the root shape position
 float ELLIPSE_X = (CANVAS_WIDTH/2); // x coordinate of the ellipse center
 float ELLIPSE_Y = (CANVAS_HEIGHT-(CANVAS_HEIGHT/10)); // y coordinate of the ellipse center
...
//variables for storing the scaling factor
float scaleX;
float scaleY;
...
void drawCopy()
{
// Drwaing the root shape whcih is the bottom ellipse
noStroke();
fill(150, 150);
...
void pencilShadow()// Adding an ellipse to represent a moving shadow
{
  noStroke();
fill(150, 150);
float shadowX = ELLIPSE_X;
float shadowY = ELLIPSE_Y;
float shadowWidth = ellipseWidth/2;
float shadowHeight = ellipseHeight/2;
float shadowXCursorDist = mouseX - shadowX;
float shadowYCursorDist = mouseY - shadowY;
shadowX = shadowX + shadowXCursorDist/10;
shadowY = shadowY + shadowYCursorDist/80;
ellipse(shadowX, shadowY, shadowWidth, shadowHeight);
}
